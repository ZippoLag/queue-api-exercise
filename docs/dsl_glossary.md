...
- **Sanitization**: the validation and safe-storage rule applied to incoming data (initial requirements: "Incoming data must be validated and sanitized"). Accepted values must be valid — non-null, non-empty, with correct types — and safe to store: strings are trimmed, values are normalized to canonical types, and everything is persisted through parameterized queries so stored values cannot alter the store's shape or inject into it. The event `payload` remains opaque: only its being a valid JSON object is enforced, its contents and format are never inspected or transformed.
