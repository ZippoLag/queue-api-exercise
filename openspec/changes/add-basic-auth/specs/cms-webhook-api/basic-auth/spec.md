...
Defines the Basic Auth (username + password) behavior required for every incoming request to the CMS Webhook API, including the reserved `cms` username and the `401`/`403` response rules.
...
The `cms` username is the only user authorized to access the CMS Webhook API. Requests authenticated with valid credentials of any other user SHALL be rejected with `403 Forbidden` and SHALL NOT execute the endpoint handler.

#### Scenario: Valid credentials for a non-cms user

- **WHEN** a client sends Basic credentials that are valid for a user other than `cms`
...
- **WHEN** a client sends Basic credentials that match the configured `cms` user
...
The `cms` user's credentials SHALL be read exclusively from environment variables. The application SHALL fail to start when the required environment variables are not set.

#### Scenario: Environment variables are set

- **WHEN** the required environment variables are set with a `cms` username and password
- **THEN** the application starts and accepts requests authenticated with those credentials

#### Scenario: Environment variables are missing

- **WHEN** any of the required environment variables is not set
- **THEN** the application fails to start with a descriptive error

### Requirement: Configured credential format

The configured `cms` username SHALL be between 10 and 20 characters in length. The password SHALL be a randomly generated GUID. A configuration that violates the username length rule SHALL cause the application to fail to start.
