...
- [x] 2.3 Verify debug mode (requires Docker on the host): the documented `-f` command boots both APIs from source with hot reload against the host `db/` stores (an entity written by a host run appears in the debug stack and vice versa); a source edit reloads; the editor can attach to a container process
